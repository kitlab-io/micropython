<template>
  ...
  <v-card-title>BEAST CAR</v-card-title>
          <!-- Tutorial_2 -->
          <v-btn class="ma-2" @click="enableHand(true)"> Enable Hand Detection </v-btn>
          <v-btn class="ma-2" @click="enableHand(false)"> Disable Hand Detection </v-btn>
          <!-- Tutorial_2 ! -->
          <v-list>
            <v-list-item>
              <v-list-item-content>
  ...
  module.exports = {
    ...
    methods: {  
      async enableHand(en)
      {
        console.log("Enable Hand: " + en);
        if(en)
        {
          await self.parent.device.send("kit._kit.enable_hand(True)\n\r");
        }
        else 
        {
          await self.parent.device.send("kit._kit.enable_hand(False)\n\r");
        }
